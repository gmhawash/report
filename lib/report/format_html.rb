class ReportTable
  ...
  def html_header(params = { })
    content_tag(:table,
                html_thead(params) + html_tfoot + yield,
                :class => ["report-table", identifier].reject(&:blank?).join(" "),
                :style => "width: #{ html_options[:width] || 'auto' };")
  end

  def html_body(params = { })
     html_table_bodies(params)
  end
  ...
  def format_html(type, data=nil, options={ })
    ...
    when :link              : content_tag(:a, format_html(*data.first), { :href => url(data[1]) }.merge(data[2] || {}))
